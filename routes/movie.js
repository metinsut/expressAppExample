...
//GET ALL MOVIES
router.get("/", (req, res) => {
  const getMovies = movieSchema.find({});
  getMovies
    .then(data => {
      res.json(data);
    })
...
router.get("/topten", (req,res) => {
  const getTopTen = movieSchema.find({}).limit(3).sort({imdb_score:-1});
  getTopTen
  .then(data => {
    if(!data) {
      res.json({message})
...
router.get("/between/:start/:end", (req,res) => {
  const getYears = movieSchema.find({
    year:{"$gte":parseInt(req.params.start),"$lte":parseInt(req.params.end)}
  });
  getYears
    .then(data => {
      res.json(data);
